...
# 可选：切换 ASR 模型（默认 qwen3-asr-flash，OpenAI 兼容协议）
...
| 🎙️ **语音** | 点击开始聆听，说出棋谱（"炮二平五"等），再点击停止识别落子；识别中按钮绿色脉冲 |
...
```text
[你说话] "炮二平五"
   ↓ 浏览器录音 (MediaRecorder → 16kHz WAV → Base64)
...
- **语音识别**：阿里云百炼 Qwen ASR（默认 `qwen3-asr-flash`，OpenAI 兼容协议，同步返回）
- **记谱解析器**：棋谱文本 → 落子坐标，支持"前/后"消歧、红黑方数字视角差异、汉字/阿拉伯数字
...
| 方案 | 识别 | 优点 | 缺点 |
|---|---|---|---|
| A. 阿里云百炼 Qwen ASR（当前实现） | 在线 | 便宜（约 0.00022 元/秒）、中文准、Key 在本地代理 | 识别需联网，下棋仍离线 |
| B. 本地 WASM ASR（规划） | 完全离线 | 全链路离线 | 需集成模型文件，工程量大 |
