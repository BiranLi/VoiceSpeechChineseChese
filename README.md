...
- 覆盖全部 7 种棋子（帅将/仕士/相象/马/车/炮/兵卒），含 "帅五进一"、"仕四进五"
- 红黑方数字视角自动换算、汉字/阿拉伯数字均可
- 同列双子用"前/后"消歧，支持标准省略列号形式（"前炮进一"）与带列号形式（"前炮二进一"）
- 车/炮/兵/帅将按步数，马/相/仕按落点列号；非法着法即时拦截
...
node test/test_notation.js      # 记谱解析器单元测试（7 棋子 + 双视角 + 前/后消歧）
